...
libD.scroll = function (opt) //direction=-1 : bottom to top; direction=1: top to bottom
{
...
libD.scroll.prototype =
{
	up : function()
	{
		if(this.currentTop > - that.child.offsetHeight)
		{
			this.currentTop -=this.offset;
			that.child.style.top=that.currentTop+"px";
	...
	down : function ()
	{
		if(that.currentTop < this.parent.offsetHeight - this.child.offsetTop)
		{
			this.currentTop +=this.offset;
	...
	_handleScroll : function(that)
	{
		...
		if(that.currentTop<=that.child.offsetHeight * that.direction)
		{
			that.currentTop=that.parent.offsetHeight;
		}
		else
		{
			that.currentTop+=that.direction;
		}
		that.child.style.top=that.currentTop+"px";		
	},
	...
	{
		if(!notPermanant) this.stoppedPermanantly = true;
