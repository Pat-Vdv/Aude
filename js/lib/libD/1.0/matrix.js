/** DEPRECATED. use css3.js instead.
...
libD.need(['numbers'], function(){
	...
	libD.getRealPos = function (mat, x, y, C, Cnd)
	{ // we calculate x' and y' with the matrix
	...
	libD.getTransformMatrixCenter = function (o, fontSize, width, height)
	{
		...
		if(torigin = style.webkitTransformOrigin)
			torigin = torigin.replace(/,/g, '.').replace(/\. /g, ','); // https://bugs.webkit.org/show_bug.cgi?id=41541 , https://bugs.webkit.org/show_bug.cgi?id=18994
		else torigin = style.transformOrigin || style.MozTransformOrigin || style.OTransformOrigin ||style.msTransformOrigin || style.khtmlTransformOrigin;
		...
		while(i < len)
		{
			switch(v[i])
			{
				...
				case 'right':C.x = width;break;
				case 'top':C.y = 0;break;
				case 'bottom':C.y=height;break;
				case 'center': break;
				default:
					if(i === 0)
...
libD.cssText = function (o)
{ //FIXME : greedy, sometimes useless vendor properties are generated for CSS 3.
