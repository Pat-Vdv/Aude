	...
	*/
	...
	this.label_onclick  = function()
	{
		if(that.onclick)
		{
			try
			{
				that.onclick(this.parentNode._path);
			}
			catch(e)
			{
...
libD.tree.prototype =
{
	append : function (p, folded)
	{
		...
		*/
		...
		if(path[i] === '' && this.ignore_root) i=1;
		if(path[len-1] === '') --len;
			...
			if(path[i] !== '' || !i)
			{
				...
				else
				{
					...
					if(folded)
					{
						...
						b.firstChild.onclick = this.a_unfold;
					}
					else
					{
					...
					b.lastChild.onclick = this.label_onclick;
	...
	remove : function (p, children)
	{
		...
		*/
		...
		{ // removing path's object from its parent
			...
			if(parentWPath.e && !parentWPath.d.firstChild) // if no more child in parent, .tree-fold -> .empty
	...
	a_fold : function ()
	{
		...
		if(this.parentNode._libD_tree.onfold)
		{
			try
			{
				this.parentNode._libD_tree.onfold(this.parentNode._path);
			}
			catch(e)
			{
				if(window.console && console.error)
	...
	unfold : function(treeE)
	{
		...
		libD.tree.prototype.a_unfold.call(treeE.e.firstChild.firstChild);
	},
	...
	fold : function(treeE)
	{
		if(typeof treeE === 'string')
			treeE = this.append(treeE);
		libD.tree.prototype.a_fold.call(treeE.e.firstChild.firstChild);
	},
	...
	a_unfold : function ()
	{
		...
		this.firstChild.textContent = this.firstChild.innerText = '-';
		if(this.parentNode._libD_tree.onunfold)
		{
			try
			{
				this.parentNode._libD_tree.onunfold(this.parentNode._path);
			}
			catch(e)
			{
				if(window.console && console.error)
