...
libD.fs.prototype = {
	...
	{
		if(this.v_count === -1)
			return this.v_count = this.content.length;
		else
			return this.v_count;
	},
	...
	{
		if(!iconPack) iconPack = this.iconPack;
...
*/
	...
	ajax_mkdir : function(p)
	{
	
	},

	_ajax_cmd : function (c, args, callback)
	{
		args.cmd = c;
...
libD.mime = {
	...
	toIcon : function(mime, path, iconPack, size, iconifier)
	{
		...
		else if(mime === 'text/x-shellscript')
			return iconDir + '/mimetypes/text-x-script.png';
		else if(mime === 'application/vnd.ms-office')
